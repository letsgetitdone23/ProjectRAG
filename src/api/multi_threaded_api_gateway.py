"""
Multi-Threaded API Gateway with Thread-Safe Session Management
Supports multiple simultaneous chat sessions with complete isolation
"""
...
# Import thread-safe session manager
import sys
sys.path.append(str(Path(__file__).parent.parent))
from session.thread_safe_session_manager import (
    get_session_manager, 
...
class ChatResponse(BaseModel):
    ...
    active_threads: int = Field(..., description="Number of active threads for this user")
    last_updated: str = Field(..., description="Last updated timestamp of the data source")
...
class MultiThreadedQueryProcessor:
    """Thread-safe query processor for Nippon India schemes"""
    
    def __init__(self):
        self.logger = logging.getLogger(__name__)
        
        # Thread-local storage for query processor instances
        self._local = threading.local()
        
        # Nippon India scheme patterns - more flexible matching
        self.nippon_patterns = {
            ...
            'multi_asset': [
                ...
                r'\bmulti\s+asset.*fund',
                r'\bmultiasset.*fund',
                r'\bmulti\s+asset.*scheme',
                r'\bmultiasset.*scheme',
                r'\bmulti\s+asset.*allocation',
                r'\bmultiasset.*allocation'
            ]
        }
        ...
        # Nippon India data (thread-safe shared data)
        self._nippon_data_lock = threading.Lock()
        self._nippon_data = {
            'large_cap': {
                'nav': '₹101.17',
                'expense_ratio': '1.25%',
            ...
            'multi_asset': {
                ...
                'riskometer': 'Moderately High',
                'fund_size': '₹2,847 Crores',
                'source_url': 'https://mf.nipponindiaim.com/FundsAndPerformance/Pages/NipponIndia-Multi-Asset-Allocation-Fund.aspx'
            }
        }
        
        self.logger.info("Multi-threaded query processor initialized")
    
    def get_nippon_data(self, scheme: str) -> Dict:
        """Thread-safe access to Nippon India data"""
        with self._nippon_data_lock:
            return self._nippon_data.get(scheme, {}).copy()
    ...
    def process_query(self, query: str, thread_id: str) -> Dict:
        """Process query with thread isolation"""
        ...
        try:
            ...
            if detected_scheme:
                return self._process_nippon_query(query, detected_scheme, thread_id)
            ...
            self.logger.error(f"Error processing query in thread {thread_id}: {str(e)}")
            return {
    ...
    def _process_nippon_query(self, query: str, scheme: str, thread_id: str) -> Dict:
        """Process Nippon India specific query with thread isolation"""
        data = self.get_nippon_data(scheme)
        query_lower = query.lower()
        ...
        elif 'fund size' in query_lower or 'fund_size' in query_lower:
            answer = f"The fund size of Nippon India {scheme.replace('_', ' ').title()} Fund Direct Growth is {data.get('fund_size', 'N/A')}."
...
class MultiThreadedAPIGateway:
    """Multi-threaded API Gateway with thread-safe session management"""
    
    def __init__(self, max_workers: int = 10):
        self.logger = logging.getLogger(__name__)
        self.query_processor = MultiThreadedQueryProcessor()
        ...
        self.logger.info(f"Multi-threaded API Gateway initialized with {max_workers} workers")
    
    def create_api(self) -> FastAPI:
        """Create FastAPI application with multi-threading support"""
        app = FastAPI(
            title="Nippon India Multi-Threaded RAG API",
            description="Thread-safe FAQ assistant for Nippon India schemes with concurrent session support",
            version="2.0.0"
        )
        ...
        async def chat_endpoint(request: ChatRequest):
            """Process chat message with thread isolation"""
    ...
    async def process_chat_request(self, request: ChatRequest) -> ChatResponse:
        """Process chat request with thread isolation"""
        start_time = time.time()
        ...
        try:
            ...
            session = get_thread_session(request.thread_id) if request.thread_id else None
            
            if not session:
                session = create_isolated_session(user_id, request.thread_id)
            ...
            # Create chat message for storage
            ...
            # Store message in thread-safe session
            add_thread_message(session.thread_id, chat_message)
            
            # Get user's active threads
            user_threads = self.session_manager.get_user_threads(user_id)
            ...
            response = ChatResponse(
                ...
                confidence=result['confidence'],
                thread_id=session.thread_id,
                session_id=session.thread_id,  # Use thread_id as session_id
                processing_time_ms=processing_time,
            ...
            self.query_count += 1
            self.logger.info(f"Processed query #{self.query_count} in thread {session.thread_id} in {processing_time:.2f}ms")
            return response
    ...
    async def get_health_status(self) -> HealthResponse:
        ...
        try:
            ...
            return HealthResponse(
                ...
                active_threads=session_stats['active_sessions'],
    ...
    def get_embedded_html(self) -> str:
        """Get embedded HTML content with multi-threading support"""
        return """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Nippon India Multi-Threaded RAG Assistant</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <!-- Header -->
        <div class="header">
            <h1>🤖 Nippon India Multi-Threaded RAG Assistant</h1>
            <div>Thread-Safe FAQ System with Concurrent Session Support</div>
        </div>

        <!-- Thread Information -->
        <div class="thread-info">
            <h3>🧵 Thread-Safe Multi-Session Support</h3>
            <p><strong>Current Thread:</strong> <span id="currentThreadId">None</span> | <strong>Active Threads:</strong> <span id="activeThreadsCount">0</span></p>
            <p>Each conversation runs in an isolated thread with no memory sharing between sessions.</p>
            <button class="new-thread-btn" onclick="createNewThread()">🆕 Create New Thread</button>
        </div>
            ...
            <div class="chat-messages" id="chatMessages">
                <div class="message assistant">
                    <div class="message-content">
                        👋 Welcome! I can help you with factual information about Nippon India mutual funds including NAV details, expense ratios, SIP amounts, exit loads, benchmark indices, and riskometer classifications. All responses are based on official sources and include source citations. This is a thread-safe session - your conversation is isolated from other users. What would you like to know?
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        window.onload = function() {
            ...
            setInterval(updateThreadInfo, 5000);
        };

        function initializeUser() {
            // Generate unique user ID
            currentUserId = 'user_' + Math.random().toString(36).substr(2, 9);
            console.log('Initialized user:', currentUserId);
        }

        function createNewThread() {
            // Create new thread ID
            currentThreadId = 'thread_' + Math.random().toString(36).substr(2, 9);
            ...
            chatMessages.innerHTML = `
                <div class="message assistant">
                    <div class="message-content">
                        🆕 New thread created! This conversation is now isolated from other sessions. Thread ID: <span class="thread-badge">${currentThreadId}</span>
                    </div>
                </div>
            `;
            
            // Update UI
            document.getElementById('currentThreadId').textContent = currentThreadId;
        ...
        async function sendMessage() {
            ...
            try {
                ...
                activeThreads = data.active_threads || 1;
                ...
                // Update UI
                document.getElementById('currentThreadId').textContent = currentThreadId;
                document.getElementById('activeThreadsCount').textContent = activeThreads;
                ...
                <div class="message-content">
                    ...
                    ${sourceUrl ? `
                        <div class="message-info">
                            ...
                            ${currentThreadId ? `<span class="thread-badge">Thread: ${currentThreadId.substr(-8)}</span>` : ''}
                        </div>
                    ` : ''}
                </div>
            `;
        ...
        async function checkAPIStatus() {
            ...
            try {
                ...
                statusElement.textContent = `✅ API Connected - ${data.active_sessions} Active Sessions`;
                statusElement.className = 'status connected';
                
                // Update thread info
                document.getElementById('activeThreadsCount').textContent = data.active_sessions;
        ...
        async function updateThreadInfo() {
            try {
                ...
                document.getElementById('activeThreadsCount').textContent = data.active_sessions || 0;
            } catch (error) {
                console.error('Failed to update thread info:', error);
            }
        }
    </script>
</body>
    ...
    def shutdown(self):
        ...
        self.logger.info("Multi-threaded API Gateway shutdown complete")

# Main function for running API
def run_multi_threaded_api_gateway(host: str = "0.0.0.0", port: int = 8000, max_workers: int = 10):
    """Run multi-threaded API Gateway"""
    gateway = MultiThreadedAPIGateway(max_workers=max_workers)
    app = gateway.create_api()
    
    print(f"🚀 Starting Nippon India Multi-Threaded RAG API...")
    print(f"🌐 Server: http://{host}:{port}")
    print(f"📊 Thread-Safe RAG System with Concurrent Sessions")
